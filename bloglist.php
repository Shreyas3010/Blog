...
<?php
		...
		//echo "aa";
?>
<form action="like.php" method="POST"><input type="hidden" name="bloggerID" value=<?php echo $BID ;?> ><input type="hidden" name="blogID" value=<?php echo $blogID ;?> ><input type="hidden" name="title" value=<?php echo $title ;?> ><button style="margin-left:140px;border:none;background-color:Transparent;"><i class="glyphicon glyphicon-heart" style="font-size:48px;color:red;text-shadow:2px 2px 4px black;"></i><font style="margin-left:10px;font-size:45px;"><?php echo $n6['likes'] ?></font></button></form>

<div><form action="writecomment.php" method="POST"><input type="hidden" name="bloggerID" value=<?php echo $BID ;?> ><input type="hidden" name="blogID" value=<?php echo $blogID ;?> ><input type="hidden" name="title" value=<?php echo $title ;?> ><input type="text" name="username" placeholder="Name" class="b" required ><input type="text" name="comment" placeholder="Comment here" class="b1" required ><button class="button3">Comment</button></form>
</div>
<details>
...
<?php
		...
		while($n5=mysqli_fetch_array($r5))
		{
			
			?>
				<p class="s2"><?php echo $n5['username'] ; ?> - <?php echo $n5['comment'] ; ?>.</p>		
		<?php
		}
